...
#warning 个人中心在tabbar的item位置
        //  消息提醒
        if (1 == i) {
            //TODO: 在此做bageValue的处理逻辑
#warning 在此做bageValue的处理逻辑
            [btn setbageValue:@"0"];
            self.specialTabBarButton = btn;  //  特殊处理的tabbar选项
...
- (void)btnOnClick:(NXTabBarButton *)btn
{
    ...
    if ([self.delegate respondsToSelector:@selector(tabBar:shouldChangeFrom:To:)]) {
        shouldChange = [self.delegate tabBar:self shouldChangeFrom:self.currentSelectedBtn.tag To:btn.tag];
    }
...
- (void)btnOnClickNew{
    NXTabBarButton *btn = self.specialTabBarButton;
    //  0. 通知代理
    if ([self.delegate respondsToSelector:@selector(tabBar:from:to:)]) {
...
- (void)selectIndex:(NSInteger)index {
    NXTabBarButton *btn = self.tabBarButtons[index];
    [self btnOnClick:btn];
}
